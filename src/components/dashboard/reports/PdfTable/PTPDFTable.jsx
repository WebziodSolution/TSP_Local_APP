...
const PTPDFTable = ({ data, companyInfo, filter, period }) => {
                    ...
                    <tbody>
                        {rows?.map((item, index) => (
                            <tr key={index} className="border border-gray-300">
                                ...
                                <td className="border border-gray-300 text-right text-sm py-4 px-4 align-middle">₹{item?.pt_amount ? item?.pt_amount?.toLocaleString('en-IN', { maximumFractionDigits: 0, minimumFractionDigits: 0 }) : 0}</td>
                            </tr>
                        ))}
                        <tr className="font-bold border border-gray-300">
                            <td colSpan={2} className="border border-gray-300 py-3 px-4 text-right text-sm capitalize align-middle">
