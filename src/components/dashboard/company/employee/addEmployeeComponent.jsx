...
const AddEmployeeComponent = ({ setAlert, handleSetTitle, handleSetUserDetails }) => {
    ...
    const handleDeleteFaceRegistration = async () => {
        setLoading(true);
        const empId = watch("employeeId") || id;
        const response = await axios.delete(`${faceRecognitionAPIBaseURL}/clear-embedding/${empId}`);
        if (response.status === 200) {
            setLoading(false);
            setAlert({ open: true, message: response.data.message, type: "success" });
            setValue("embedding", null);
            if (id) {
                handleGetEmployee();
            }
    ...
    const handleGetEmployeeRoles = async () => {
        if (steps[activeStep] === "Employee Info" && userInfo?.companyId) {
    ...
    const handleGetAllUserType = async () => {
        if (steps[activeStep] === "Employee Info") {
    ...
    const handleGetAllShift = async () => {
        if (steps[activeStep] === "Employee Info" && companyId) {
            ...
            const data = res?.data?.result?.map((item) => {
                ...
                const timeDisplay = isHourly
                    ? `${item.totalHours} hrs`
                    : `${dayjs(item?.startTime).format("hh:mm A")} - ${dayjs(item?.endTime).format("hh:mm A")}`;
    ...
    const handleGetAllOvertimeRules = async () => {
        if (steps[activeStep] === "PayRoll Info" && userInfo?.companyId) {
    ...
    const handleGetAllWeeklyOff = async () => {
        if (userInfo?.companyId && steps[activeStep] === "PayRoll Info") {
    ...
    const handleGetAllDeductions = async () => {
        if (steps[activeStep] === "Allowance & Deduction") {
    ...
    const submit = async (data, submitType) => {
        ...
        const newData = {
            ...
            gender: parseInt(watch("gender")) === 1 ? "Male" : parseInt(watch("gender")) === 2 ? "Female" : "",
        ...
        if (steps[activeStep] === "Employee Info") {
            if (id || watch("employeeId")) {
                ...
                const isSelfUpdate = loggedInUser && (
                    String(loggedInUser.employeeId) === String(id || watch("employeeId")) ||
                    String(loggedInUser.userId) === String(id || watch("employeeId"))
                );

                if (isSelfUpdate && (oldData?.userName !== watch("userName") || oldData?.password !== watch("password"))) {
        ...
        else if (steps[activeStep] === "Face Registration") {
            if (saveAndExit) {
                handleSetTitle("Manage Employees");
                navigate("/dashboard/manageemployees");
            } else {
        ...
        else if (steps[activeStep] === "Employment Info" || steps[activeStep] === "PayRoll Info") {
            const empId = id || watch("employeeId");
            if (empId) {
                if (oldData?.userName !== watch("userName") || oldData?.password !== watch("password")) {
                    handleOpenDialogLogin(newData);
                    return;
                }
                const res = await updateEmployee(empId, newData)
                if (res.data?.status === 200) {
                    if (saveAndExit) {
                        handleSetTitle("Manage Employees");
                        navigate("/dashboard/manageemployees");
        ...
        else if (steps[activeStep] === "Direct Deposit") {
            if (watch("accountId")) {
    ...
    return (
        <div className='px-3 lg:px-0'>
            <div className='border rounded-lg bg-white lg:w-full p-5'>
                <div className='flex justify-start items-start gap-6 relative'>
                    ...
                    <div className='py-3 xl:w-[800px] xl:px-24'>
                        ...
                        >
                            {
                                steps[activeStep] === "Employee Info" && (
                                    <>
                                        ...
                                        <div className='grid grid-cols-2 lg:grid-cols-2 gap-6'>
                                            ...
                                            {
                                                (watch("employeeTypeId") !== null && parseInt(watch("employeeTypeId")) !== 3) && (
                                                    <div>
                                                        <Controller
                                                            name="hourlyRate"
                                                            control={control}
                                            ...
                                            {(watch("employeeTypeId") != null && watch("employeeTypeId") === 3) && (
                                                <div>
                                                    <Controller
                                                        name="basicSalary"
                                                        control={control}
...

                                            {
                                                (watch("embedding") && id) ? (
                                                    <div className='flex items-center justify-start'>
                                                        <Button useFor='error' type='button' text={'Delete Registered Face'} onClick={handleOpenFaceRegistrationDialog} />
                                                    </div>
                                                ) : id ? (
                                                    <div className='flex items-center justify-start'>
                                                        <Button type='button' text={'Set Face Registration'} onClick={handleOpenFaceRegistration} />
                                                    </div>
                                                ) : null
                                            }
                            ...
                            {
                                steps[activeStep] === "Face Registration" && (
                                    <>
                                                ...
                                                <>
                                                    ...
                                                    <div className="flex justify-center">
                                                        <Button
                                                            ...
                                                            onClick={handleOpenFaceRegistrationDialog}
                                                        />
                                                    </div>
                                                </>
                                            ) : (
                            ...
                            {
                                steps[activeStep] === "Employment Info" && (
                                    <>
                                        <div className='mb-2 font-medium text-center text-[28px] text-[#262B43]'>
                                        ...
                                        <div className='my-6 text-base font-medium leading-snug text-[#262B43]'>
                                            <p style={{ color: theme.palette.primary.text.main, }}>Hiring & Profile Information</p>
                                        </div>
                                        ...
                                        <div className='my-6 text-base font-medium leading-snug text-[#262B43]'>
                                            <p style={{ color: theme.palette.primary.text.main, }}>Geo-Fencing & Penalty Settings</p>
                                        </div>
                                ...
                                steps[activeStep] === "PayRoll Info" && (
                                    <>
                                        ...
                                        <div className='grid grid-cols-2 gap-6'>
                                            ...
                                            {
                                                (watch("canteenType") && parseInt(watch("canteenType")) !== 3) && (
                                                    <div>
                                                        <Controller
                                                            name="canteenAmount"
                                                            control={control}
                            ...
                            {
                                steps[activeStep] === "Allowance & Deduction" && (
                                    <>
                            ...
                            {
                                steps[activeStep] === "Direct Deposit" && (
                                    <>
                                        <div className='mb-2 font-medium text-center text-[28px] text-[#262B43]'>
                            ...
                            <div className='flex justify-end gap-3 mt-6'>
                                <div>
                                    <Button useFor='disabled' type={'button'} text={"Cancel"} onClick={handleBackToManageEmployee} />
                                </div>
                                <div>
                                    <Button useFor='disabled' type={'button'} text={"Back"} onClick={handleBack} disabled={activeStep === 0} />
                                </div>
                                <div>
                                    <Button value="save" type={'submit'} text={activeStep === steps.length - 1 ? "Submit" : "Next"} isLoading={loading} />
                                </div>
            ...
            <FaceRegistration
                ...
                handleClose={handleCloseFaceRegistration}
                employeeId={watch("employeeId") || id}
                type="register"
                onSuccess={(data) => {
                    setValue("embedding", "registered");
                    setActiveStep((prev) => prev + 1);
                }}
            />
        </div>
